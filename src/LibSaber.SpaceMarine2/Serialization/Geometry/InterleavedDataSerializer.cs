...
namespace LibSaber.SpaceMarine2.Serialization.Geometry
{
  ...
  public sealed class InterleavedDataSerializer : GeometryElementSerializer<InterleavedData>
  {
    ...
    public InterleavedEnumerator GetEnumerator(SpanReader reader, int startIndex, int endIndex)
    {
      var startOffset = Buffer.StartOffset + (startIndex * Buffer.ElementSize);
      var length = endIndex - startIndex;

      return new InterleavedEnumerator(Buffer, reader, (uint)length);
    }
  ...
  public ref struct InterleavedEnumerator
  {
    #region Data Members

    private FVFFlags Flags;
    private SpanReader reader;
    private int _stride;

    private uint _currentIdx;
    private uint _count;

    private InterleavedData data;

    #endregion

    public InterleavedData Current => data;

    public InterleavedEnumerator(GeometryBuffer buffer,
      SpanReader reader,
      uint count)
    {
      Flags = buffer.FlagSet;
      this.reader = reader;
      _stride = buffer.ElementSize;

      _currentIdx = uint.MaxValue;
      _count = count;
    }

    public bool MoveNext()
    {
      if (unchecked(++_currentIdx < _count))
      {
        data = default;
        ...
        ASSERT(lenRead == _stride, "Incorrect buffer element read.");
    ...
    private void ReadTangents(ref InterleavedData data)
    {
      ...
      if ((Flags & FVFFlags.TANG_COMPR) != 0)
      {
        if (data.HasTangent0) data.Tangent0 = ReadCompressedTangent();
        if (data.HasTangent1) data.Tangent1 = ReadCompressedTangent();
        if (data.HasTangent2) data.Tangent2 = ReadCompressedTangent();
        if (data.HasTangent3) data.Tangent3 = ReadCompressedTangent();
        if (data.HasTangent4) data.Tangent4 = ReadCompressedTangent();
      }
      else
      {
        if (data.HasTangent0) data.Tangent0 = ReadUncompressedTangent();
        if (data.HasTangent1) data.Tangent1 = ReadUncompressedTangent();
        if (data.HasTangent2) data.Tangent2 = ReadUncompressedTangent();
        if (data.HasTangent3) data.Tangent3 = ReadUncompressedTangent();
        if (data.HasTangent4) data.Tangent4 = ReadUncompressedTangent();
      }
    }

    [MethodImpl(Globals.METHOD_IMPL_AGGRESSIVE)]
    private Vector4 ReadCompressedTangent()
    {
      var x = reader.ReadSByte().SNormToFloat();
      var y = reader.ReadSByte().SNormToFloat();
      var z = reader.ReadSByte().SNormToFloat();
    ...
    [MethodImpl(Globals.METHOD_IMPL_AGGRESSIVE)]
    private Vector4 ReadUncompressedTangent()
    {
      var x = reader.ReadFloat32();
      var y = reader.ReadFloat32();
      var z = reader.ReadFloat32();
    ...
    private void ReadVertexColors(ref InterleavedData data)
    {
      ...
      if(data.HasColor0) data.Color0 = ReadVertexColor();
      if(data.HasColor1) data.Color1 = ReadVertexColor();
      if(data.HasColor2) data.Color2 = ReadVertexColor();
      if(data.HasColor3) data.Color3 = ReadVertexColor();
      if(data.HasColor4) data.Color4 = ReadVertexColor();
      if(data.HasColor5) data.Color5 = ReadVertexColor();
    }
    ...
    private void ReadUVs(ref InterleavedData data)
    {
      ...
      {
        data.HasUV0 = true;
        data.UV0 = ReadUV((Flags & FVFFlags.TEX0_COMPR) != 0);
        ASSERT((Flags & FVFFlags.TEX0_4D) == 0, "4D UV");
      }
      if ((Flags & FVFFlags.TEX1) != 0)
      {
        data.HasUV1 = true;
        data.UV1 = ReadUV((Flags & FVFFlags.TEX1_COMPR) != 0);
        ASSERT((Flags & FVFFlags.TEX1_4D) == 0, "4D UV");
      }
      if ((Flags & FVFFlags.TEX2) != 0)
      {
        data.HasUV2 = true;
        data.UV2 = ReadUV((Flags & FVFFlags.TEX2_COMPR) != 0);
        ASSERT((Flags & FVFFlags.TEX2_4D) == 0, "4D UV");
      }
      if ((Flags & FVFFlags.TEX3) != 0)
      {
        data.HasUV3 = true;
        data.UV3 = ReadUV((Flags & FVFFlags.TEX3_COMPR) != 0);
        ASSERT((Flags & FVFFlags.TEX3_4D) == 0, "4D UV");
      }
      if ((Flags & FVFFlags.TEX4) != 0)
      {
        data.HasUV4 = true;
        data.UV4 = ReadUV((Flags & FVFFlags.TEX4_COMPR) != 0);
        ASSERT((Flags & FVFFlags.TEX4_4D) == 0, "4D UV");
      }
      if ((Flags & FVFFlags.TEX5) != 0)
      {
        data.HasUV5 = true;
        data.UV5 = ReadUV((Flags & FVFFlags.TEX5_COMPR) != 0);
        ASSERT((Flags & FVFFlags.TEX5_4D) == 0, "4D UV");
      }
    }
