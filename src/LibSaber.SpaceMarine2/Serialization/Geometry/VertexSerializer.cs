...
namespace LibSaber.SpaceMarine2.Serialization.Geometry
{
  ...
  public sealed class VertexSerializer : GeometryElementSerializer<Vertex>
  {
    ...
    public VertexEnumerator GetEnumerator(SpanReader reader, int startIndex, int endIndex)
    {
      var startOffset = Buffer.StartOffset + (startIndex * Buffer.ElementSize);
      var length = endIndex - startIndex;

      return new VertexEnumerator(Buffer, reader, _position, _scale, (uint)length);
    }
  ...
  public ref struct VertexEnumerator
  {
    ...
    public Vertex Current => vertex;

    public VertexEnumerator(GeometryBuffer buffer, 
      SpanReader reader,
      Vector3 position, 
      Vector3 scale,
      uint count)
    ...
    [MethodImpl(Globals.METHOD_IMPL_AGGRESSIVE)]
    private void ReadMaskingFlags(ref Vertex vertex)
    {
      if ((Flags & FVFFlags.MASKING_FLAGS) != 0)
      {
        _ = reader.ReadInt32();
